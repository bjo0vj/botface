...
// Đã xóa đoạn code ghi đè require gây lỗi
...
module.exports.handleEvent = async function ({ event, api, Users }) {
    ...
    // if (api.getCurrentUserID()==senderID)return; // Đã bỏ để bot có thể tự rep tin nhắn chính mình
    if (!global.moduleData.shortcut) global.moduleData.shortcut = new Map();
    ...
    if ((dataThread = mentions.length > 0 ? data.find(item => typeof item.tag_id == 'string' && mentions.includes(item.tag_id)) : false) || (dataThread = data.find(item => (item.input || '').toLowerCase() == body.toLowerCase()))) {
        ...
        // Giả sử global.khanhdayr đã có giá trị trước đó (được gán từ api_link() hoặc ở đâu đó khác)
        // console.log(global.khanhdayr);  // Đã tắt debug log

        // Xử lý nội dung tin nhắn (msg) và các đính kèm
        if (dataThread.uri === 'random') {
            msg.attachment = [await stream_url(global.khanhdayr)];  // Gọi trực tiếp global.khanhdayr
        } else if (/^https:\/\//.test(dataThread.uri)) {
            msg.attachment = [await stream_url(dataThread.uri)];
        }

        // Kiểm tra xem msg có đính kèm hay không
        // console.log(msg.attachment);  // Đã tắt debug log

        // Trả lại tin nhắn với nội dung và đính kèm
        return api.sendMessage({
            body: msg.body,  // Nội dung của tin nhắn (msg.body)
            attachment: global.khanhdayr.splice(0, 1)  // Đính kèm từ global.khanhdayr
        }, threadID, messageID);
    }
}
