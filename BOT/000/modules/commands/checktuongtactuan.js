...
module.exports.config = {
    ...
    hasPermssion: 0,
    credits: "Bot",
    description: "Xem thành viên có tương tác tuần lớn hơn số nhập vào",
    commandCategory: "Người dùng",
    usages: "[số] - Lọc thành viên có tương tác tuần > số nhập",
    cooldowns: 3
};
...
module.exports.run = async function ({ api, event, args }) {
    ...
    // Lọc thành viên có tương tác tuần > minCount
    const filtered = members.filter(m => (m.week || 0) > minCount);

    if (filtered.length === 0) {
        return api.sendMessage(
            `[ TƯƠNG TÁC TUẦN > ${minCount} ]\n` +
            `────────────────────\n` +
    ...
    for (const member of sorted) {
        ...
        lines.push(`${count}. ${userName} | Tuần: ${member.week || 0} | Cuối: ${lastTime}`);
        count++;
    }
