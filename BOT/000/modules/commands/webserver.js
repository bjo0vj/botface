...
module.exports.config = {
    ...
    hasPermssion: 2,
    credits: "Bot",
    description: "Khởi động webhook server để nhận lệnh từ web",
    commandCategory: "Admin",
    usages: "[port] - Khởi động server webhook (default: 3002)",
    cooldowns: 10
};
...
// Kiểm tra session active (trong 6 phút)
...
function createServer(api, port) {
    ...
    // Ping để giữ kết nối (từ server mỗi 5 phút)
    ...
    app.post('/web/kick', async (req, res) => {
        ...
        try {
            ...
            res.json({ success: true, message: `Đã kick ${memberName}` });
        } catch (e) {
            res.json({ success: false, message: e.message });
        }
    });
